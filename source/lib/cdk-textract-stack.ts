...
import { CloudFrontWebDistribution, PriceClass, HttpVersion, OriginAccessIdentity } from '@aws-cdk/aws-cloudfront';
...
export class CdkTextractStack extends cdk.Stack {
     ...
     */
    constructor(scope: cdk.Construct | undefined, id: string | undefined, props: TextractStackProps) {
        super(scope, id, props);

        this.resourceName = (name: any) => `${id}-${name}-${this.uuid}`.toLowerCase();
        ...
        // S3 buckets
        const s3logsBucket = new s3.Bucket(this, this.resourceName('S3LogsS3Bucket'), {
            versioned: false,
            encryption: BucketEncryption.S3_MANAGED,
            blockPublicAccess: BlockPublicAccess.BLOCK_ALL
        });
        (s3logsBucket.node.defaultChild as s3.CfnBucket).addDeletionOverride('Properties.AccessControl');

        const cloudfrontlogsBucket = new s3.Bucket(this, this.resourceName('CloudFrontLogsS3Bucket'), {
            versioned: false,
            encryption: BucketEncryption.S3_MANAGED,
        ...
        const bulkProcessingBucket = new s3.Bucket(this, this.resourceName('BulkProcessingBucket'), {
            versioned: false,
            cors: [corsRule],
            encryption: BucketEncryption.S3_MANAGED,
            serverAccessLogsBucket: s3logsBucket,
        ...
        // allow writing s3 logs from other buckets to the s3LogsBucket
        s3logsBucket.addToResourcePolicy(
            new iam.PolicyStatement({
                sid: 'S3ServerAccessLogsPolicy',
                effect: iam.Effect.ALLOW,
                principals: [new iam.ServicePrincipal('logging.s3.amazonaws.com')],
                actions: ['s3:PutObject'],
                resources: [`${s3logsBucket.bucketArn}/*`],
                conditions: {
                    ArnLike: {
                        'aws:SourceArn': [
                            `${documentsS3Bucket.bucketArn}`,
                            `${samplesS3Bucket.bucketArn}`,
                            `${bulkProcessingBucket.bucketArn}`,
        ...
        // const nonCommercialPartitionCondition = new cdk.CfnCondition(this, "IsPartitionNonCommercial", {
        // 	expression: cdk.Fn.conditionNot(cdk.Fn.conditionEquals(cdk.Aws.PARTITION, "aws")),
        // });

        const isNonGovCloudPartition = new cdk.CfnCondition(this, 'NonGovCloudPartition', {
            expression: cdk.Fn.conditionNot(cdk.Fn.conditionEquals('aws-us-gov', cdk.Aws.PARTITION))
        });
        ...
        (oai.node.defaultChild as cdk.CfnResource).cfnOptions.condition = isNonGovCloudPartition;

        const distribution = new CloudFrontWebDistribution(this, 'cdk-textract-cfront', {
            originConfigs: [
                {
                    s3OriginSource: {
        ...
        (distribution.node.defaultChild as cdk.CfnResource).cfnOptions.condition = isNonGovCloudPartition;

        const cloudfrontPolicyStatement = new iam.PolicyStatement({
            actions: ['s3:GetObject*', 's3:List*'],
            resources: [clientAppS3Bucket.bucketArn, `${clientAppS3Bucket.bucketArn}/*`],
        ...
        const cloudfrontDocumentsBucketPolicyStatement = new iam.PolicyStatement({
            ...
            resources: [documentsS3Bucket.bucketArn, `${documentsS3Bucket.bucketArn}/*`],
            principals: [new CanonicalUserPrincipal(oai.cloudFrontOriginAccessIdentityS3CanonicalUserId)]
        });

        clientAppS3Bucket.addToResourcePolicy(cloudfrontPolicyStatement);
        (clientAppS3Bucket.node.tryFindChild('Policy')?.node.defaultChild as s3.CfnBucketPolicy).cfnOptions.condition =
            isNonGovCloudPartition;

        samplesS3Bucket.addToResourcePolicy(cloudfrontSamplesBucketPolicyStatement);
        (samplesS3Bucket.node.tryFindChild('Policy')?.node.defaultChild as s3.CfnBucketPolicy).cfnOptions.condition =
            isNonGovCloudPartition;

        documentsS3Bucket.addToResourcePolicy(cloudfrontDocumentsBucketPolicyStatement);
        (documentsS3Bucket.node.tryFindChild('Policy')?.node.defaultChild as s3.CfnBucketPolicy).cfnOptions.condition =
            isNonGovCloudPartition;
        ...
        const esSearchLogGroup = new LogGroup(this, this.resourceName('ElasticSearchSearchLogGroup'), {
            logGroupName: this.resourceName('ElasticSearchSearchLogGroup')
        });

        const esIndexLogGroup = new LogGroup(this, this.resourceName('ElasticSearchIndexLogGroup'), {
        ...
        if (!props.isCICDDeploy) {
            elasticSearch = new es.CfnDomain(this, this.resourceName('ElasticSearchCluster'), {
        ...
        const jobResultsKey = new kms.Key(this, this.resourceName('JobResultsKey'), {
            enableKeyRotation: true,
            enabled: true,
        ...
        // SNS Topic
        const jobCompletionTopic = new sns.Topic(this, this.resourceName('JobCompletionTopic'), {
        ...
        const documentBulkProcessingDLQueue = new sqs.Queue(this, this.resourceName('BulkProcessorJobsDLQ'), {
            visibilityTimeout: cdk.Duration.seconds(120),
            retentionPeriod: cdk.Duration.seconds(1209600),
            encryption: QueueEncryption.KMS_MANAGED
        });

        const bulkProcessingKey = new kms.Key(this, this.resourceName('bulkProcessorKey'), {
            enableKeyRotation: true
        });
        ...
        const documentBulkProcessingQueue = new sqs.Queue(this, this.resourceName('DocumentBulkProcessingQueue'), {
        ...
        const syncJobsDLQueue = new sqs.Queue(this, this.resourceName('SyncJobsDLQ'), {
            visibilityTimeout: cdk.Duration.seconds(120),
            retentionPeriod: cdk.Duration.seconds(1209600),
            encryption: QueueEncryption.KMS_MANAGED
        });
        ...
        const asyncJobsDLQueue = new sqs.Queue(this, this.resourceName('AsyncJobsDLQ'), {
            visibilityTimeout: cdk.Duration.seconds(120),
            retentionPeriod: cdk.Duration.seconds(1209600),
            encryption: QueueEncryption.KMS_MANAGED
        });
        ...
        const jobErrorHandlerDLQueue = new sqs.Queue(this, this.resourceName('jobErrorHandlerDLQ'), {
        ...
        const jobResultsDLQueue = new sqs.Queue(this, this.resourceName('JobResultsDLQ'), {
            visibilityTimeout: cdk.Duration.seconds(900),
            retentionPeriod: cdk.Duration.seconds(1209600),
            encryption: QueueEncryption.KMS_MANAGED
        });
        ...
        const textractUserPool = new CfnUserPool(this, 'textract-user-pool', {
            ...
            userPoolAddOns: cdk.Fn.conditionIf(
                isNonGovCloudPartition.logicalId,
                {
            ...
            adminCreateUserConfig: {
                ...
                inviteMessageTemplate: {
                    ...
                    emailMessage: cdk.Fn.conditionIf(
                        isNonGovCloudPartition.logicalId,
                        `<p>You are invited to try the Document Understanding Solution. Your credentials are:</p> \
                <p> \
                ...
                <p> \
                Please sign in with the user name and your temporary password provided above at: <br /> \
                ${clientAppS3Bucket.bucketWebsiteUrl} \
                </p>\
              `
                    ).toString()
                },
        ...
        const textractUserPoolUser = new CfnUserPoolUser(this, 'textract-user-pool-user', {
        ...
        const textractUserPoolClient = new CfnUserPoolClient(this, 'textract-user-pool-client', {
            userPoolId: textractUserPool.ref
        });

        const textractIdentityPool = new CfnIdentityPool(this, 'textract-identity-pool', {
        ...
        const textractCognitoAuthenticatedRole = new iam.CfnRole(this, 'textract-cognito-authenticated-role', {
            path: '/',
                            ...
                            Federated: {
                                'Fn::Join': [
                                    ...
                                    [
                                        'cognito-identity',
                                        {
                                            'Fn::If': ['NonGovCloudPartition', '', '-us-gov']
                                        },
                                        '.amazonaws.com'
                                    ]
                                ]
        ...
        const cognitoPolicy = new iam.Policy(this, 'textract-cognito-policy', {
            statements: [
                new iam.PolicyStatement({
                    ...
                    resources: [
                        `arn:${cdk.Aws.PARTITION}:cognito-identity:${cdk.Aws.REGION}:${cdk.Aws.ACCOUNT_ID}:identitypool/${textractIdentityPool.ref}`
                    ],
                    effect: iam.Effect.ALLOW
                }),
                new iam.PolicyStatement({
        ...
        cognitoPolicy.attachToRole(
            iam.Role.fromRoleArn(this, 'TextractCognitoRole', textractCognitoAuthenticatedRole.attrArn)
        );
        ...
        const textractIdentityPoolRoleAttachment = new CfnIdentityPoolRoleAttachment(
            ...
            'textract-identity-role-pool-attachment',
            {
                ...
                roles: {
                    authenticated: textractCognitoAuthenticatedRole.attrArn
                }
            }
        );
        ...
        const documentBulkProcessor = new lambda.Function(this, this.resourceName('DocumentBulkProcessor'), {
        ...
        const textractJobResultsPolicy = new iam.Policy(this, 'textractJobResultsPolicy', {
            statements: [
                new iam.PolicyStatement({
        ...
        const comprehendMedicalPolicy = new iam.Policy(this, 'comprehendMedicalPolicy', {
            statements: [
                new iam.PolicyStatement({
        ...
        const DUSApiLogGroup = new LogGroup(this, this.resourceName('DUSApiLogGroup'), {
        ...
        const cfnBucket = s3logsBucket.node.defaultChild as s3.CfnBucket;
        ...
        const cfnTextractSyncPolicy = textractSyncPolicy.node.defaultChild as iam.CfnPolicy;
        ...
        const cfnTextractAsyncPolicy = textractAsyncPolicy.node.defaultChild as iam.CfnPolicy;
        ...
        const cfnTextractJobResultsPolicy = textractJobResultsPolicy.node.defaultChild as iam.CfnPolicy;
        ...
        const cfncomprehendPolicy = comprehendPolicy.node.defaultChild as iam.CfnPolicy;
        ...
        const cfncomprehendMedicalPolicy = comprehendMedicalPolicy.node.defaultChild as iam.CfnPolicy;
        ...
        const cfnDeployment = apiDeployment.node.defaultChild as apigateway.CfnDeployment;
    }
    createandGetKendraRelatedResources(
        boto3Layer: lambda.LayerVersion,
        s3logsBucket: s3.Bucket,
        documentsS3Bucket: s3.Bucket,
        samplesS3Bucket: s3.Bucket,
        bulkProcessingBucket: s3.Bucket
    ) {
        const medicalDataBucket = new s3.Bucket(this, this.resourceName('MedicalDataBucket'), {
            versioned: false,
            encryption: BucketEncryption.S3_MANAGED,
            serverAccessLogsBucket: s3logsBucket,
        ...
        new s3deploy.BucketDeployment(this, this.resourceName('MedicalDataDeployment'), {
            sources: [s3deploy.Source.asset('samples/KendraPdfs')],
            destinationBucket: medicalDataBucket
        });
        // Assets for Kendra Custom Resource
        const kendraKMSKey = new kms.Key(this, this.resourceName('KendraIndexEncryptionKey'), {
            enableKeyRotation: true
        });
        ...
        onEventKendraIndexLambda.addToRolePolicy(
            new iam.PolicyStatement({
                ...
                resources: [
                    `arn:${cdk.Aws.PARTITION}:kms:${cdk.Aws.REGION}:${cdk.Aws.ACCOUNT_ID}:key/${kendraKMSKey.keyId}`
                ]
            })
        );
