...
export class CdkTextractStack extends cdk.Stack {
	...
	constructor(scope: cdk.Construct | undefined, id: string | undefined, props: TextractStackProps) {
		...
		// S3 buckets
		const logsS3Bucket = new s3.Bucket(this, this.resourceName("LogsS3Bucket"), {
		...
		cognitoPolicy.attachToRole(iam.Role.fromRoleArn(this, 'TextractCognitoRole', textractCognitoAuthenticatedRole.attrArn));
	...
	createandGetKendraRelatedResources(
		...
		kendraRole.addToPolicy(
			new iam.PolicyStatement({
				...
				resources: [`arn:${cdk.Aws.PARTITION}:logs:${cdk.Aws.REGION}:${cdk.Aws.ACCOUNT_ID}:log-group:/aws/kendra/*`],
			})
		);
		...
		onEventKendraIndexLambda.addToRolePolicy(
			new iam.PolicyStatement({
				...
				resources: [`arn:${cdk.Aws.PARTITION}:kms:${cdk.Aws.REGION}:${cdk.Aws.ACCOUNT_ID}:key/${kendraKMSKey.keyId}`],
			})
		);
